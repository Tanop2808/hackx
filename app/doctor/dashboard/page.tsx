  ...
  return (
    <div style={{ background: "#0d1520", minHeight: "100vh", display: "flex", justifyContent: "center" }}>
      <div style={{ width: 390, background: C.bg, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        {/* Stats */}
        <div style={{ display: "flex" }}>
          {[{ n: redCount, l: "🔴 RED", bg: "#FDEDED", c: C.red }, { n: yellowCount, l: "🟡 YELLOW", bg: "#FEF9E7", c: "#B7770D" }, { n: greenCount, l: "🟢 GREEN", bg: "#E8F8EF", c: C.green }, { n: queue.length, l: "📋 Total", bg: C.bg, c: C.muted }].map((s, i) => (
            <div key={i} style={{ flex: 1, padding: "8px 4px", textAlign: "center", background: s.bg, borderBottom: `2px solid ${C.border}` }}>
              <div style={{ fontSize: 18, fontWeight: 800, color: s.c }}>{s.n}</div>
              <div style={{ fontSize: 10, color: s.c, fontWeight: 700 }}>{s.l}</div>
            </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "10px 16px 16px" }}>
            {loadingQueue && (
              <div style={{ textAlign: "center", padding: "30px 20px", color: C.muted }}>
                ...
                <div style={{ fontSize: 13 }}>Queue लोड हो रही है...</div>
              </div>
            )}
            {!loadingQueue && queue.length === 0 && (
              <div style={{ textAlign: "center", padding: "40px 20px", color: C.muted }}>
                <div style={{ fontSize: 40 }}>✅</div>
                <div style={{ fontSize: 15, fontWeight: 700, marginTop: 12 }}>Queue is clear!</div>
                <div style={{ fontSize: 12, marginTop: 6 }}>कोई pending मरीज़ नहीं</div>
              </div>
            )}
            {queue.map((p) => {
              const initial = URGENCY_INITIAL[p.urgency] || p.patientName[0]?.toUpperCase() || "?";
              ...
              const aiNote = (p.triageResult as { summary?: string; aiSummary?: string })?.summary ||
                             (p.triageResult as { summary?: string; aiSummary?: string })?.aiSummary || "";
              return (
                <div key={p._id} style={{ background: C.card, borderRadius: 16, padding: 14, marginBottom: 10, border: `1px solid ${C.border}`, borderLeft: `4px solid ${avC}` }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: avBg, border: `2px solid ${avC}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 800, color: avC, flexShrink: 0 }}>{initial}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14, fontWeight: 800 }}>
                        {p.patientName} <span style={{ display: "inline-flex", alignItems: "center", padding: "3px 8px", borderRadius: 20, fontSize: 11, fontWeight: 700, ...BADGE[p.urgency] }}>
                          {p.urgency === "RED" ? "🔴 RED" : p.urgency === "YELLOW" ? "🟡 YELLOW" : "🟢 GREEN"}
                        </span>
                      </div>
                      <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>
                        📱 {p.patientPhone}{p.slot ? ` · 🕐 ${p.slot}` : ""}{p.queueNo ? ` · #${p.queueNo}` : ""}
                      </div>
                    </div>
                  </div>
                  {p.symptoms.length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 5, margin: "8px 0" }}>
                      {p.symptoms.slice(0, 4).map((sym, j) => (
                        <span key={j} style={{ background: C.bg, borderRadius: 6, padding: "3px 8px", fontSize: 11, fontWeight: 600 }}>{sym}</span>
                      ))}
                      {p.symptoms.length > 4 && <span style={{ background: C.bg, borderRadius: 6, padding: "3px 8px", fontSize: 11, color: C.muted }}>+{p.symptoms.length - 4} more</span>}
                    </div>
                  )}
                  {aiNote ? <div style={{ fontSize: 12, color: C.muted, fontStyle: "italic", marginBottom: 8 }}>🤖 {aiNote}</div> : null}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 11, color: C.muted }}>⏱️ {timeAgo}</span>
                    <div style={{ display: "flex", gap: 6 }}>
                      <button
                        onClick={() => markDone(p._id)}
                        disabled={markingDone === p._id}
                        style={{ padding: "7px 12px", borderRadius: 8, border: "none", background: "#E8F8EF", color: C.green, fontSize: 12, fontWeight: 700, cursor: "pointer", opacity: markingDone === p._id ? 0.6 : 1 }}>
                        {markingDone === p._id ? "..." : "✓ Done"}
                      </button>
                      <button onClick={() => router.push(`/doctor/consultation/${p._id}`)} style={{ padding: "7px 12px", borderRadius: 8, border: "none", background: C.primaryDark, color: "white", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Open →</button>
                    </div>
                  </div>
                </div>
              );
